...
## 4. Tracing Asynchronous Chains and Object Lifetimes

Many of the most important Chromium bugs don't happen on the exact line that was edited—they happen two or three asynchronous hops away when a callback fires after an object was reset or destroyed.

If an agent has to manually search the repository for every caller, destructor, and `BindOnce` site, it spends most of its time running searches instead of reasoning about lifetimes. To make async tracing reliable, [`scripts/build-caller-index.py`](scripts/build-caller-index.py) precomputes two sets of files before discovery starts:

1. **Repository-Wide Caller Lists:** Every caller of every changed function across the repository, so agents can immediately see who calls the modified code.
2. **2-Hop Class Lifetime Dossiers:** For every C++ class modified by the CL, the script inspects the class's `.h` and `.cc` files and assembles a single summary document containing:
...
| :--- | :--- |
| **Context Agent** | Reads linked bugs, design docs, and the CL description so reviewers know *what problem the author is trying to solve* before judging the code. (Skipped automatically when a small CL has no external links.) |
| **Inventory Agent** | Lists every changed function, method, constructor, destructor, and diff hunk in the CL, and maps out how they connect. This guarantees that no part of a multi-file diff is accidentally skipped. |
| **Prior-Feedback Agent** | *(Used on follow-up patchsets)* Checks each comment from previous review rounds and unresolved Gerrit threads to verify whether the fix actually resolved the issue, or if repeated patchset edits ("fix-on-fix churn") introduced new bugs or left behind dead code. |
| **Planner Agent** | Used on large CLs (or when round-two specialists are needed) to divide large diffs into balanced chunks that fit comfortably inside each subagent's context window. |
| **Verification Skeptics** | The main filter against false positives. Each Skeptic takes a group of candidate issues from Discovery and tries to *disprove* them by checking real callers, preconditions, locks, and tests. Only issues proven reachable survive. |
| **Invariant Affinity Auditor** | Checks all surviving findings side-by-side to make sure the review doesn't contradict itself or report the same underlying bug three different ways. |
| **Root-Cause Challenger** | Reviews every confirmed bug and its suggested fix to make sure we recommend fixing the actual owner of the broken invariant rather than suggesting a fragile caller workaround or unnecessary refactoring. |
...
| **Changed-Lines Polish & Holistic** | • Naming clarity, stale comments, direct `#include` hygiene, and local directory conventions.<br/>• Steps back to check whether the CL as a whole cleanly solves the stated bug without leaving dead plumbing. | Keeps minor readability nits separate from correctness bugs, and checks the overall shape of the CL. |
