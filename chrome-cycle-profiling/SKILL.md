...
* **Redundancy Probe:** `resources/redundancy_probe.h` counts calls, applicable calls and repeated inputs per scored group and times each call through `RedundancyScope` (exclusive of nested scopes of the same counter, stamped with the executable's GNU build id); reduce its `[SP3_REDUNDANCY_ROW]` lines with `optimize-campaign/scripts/redundancy_evidence.py` before proposing a skip-the-subtree or reuse-a-result mechanism. Rebuild the twin and re-run after any change to the header or the probe patch: the gate ties every packet to the build that logged it.
