...
- PASS reviews cite an existing artifact or bound digest plus a number, with distinct evidence per check. Profile-gate reviews use the `profile-review-scaffold` checks; every gate review's transcript must resolve on the ledger host (`--gate-<role>-transcript`).
