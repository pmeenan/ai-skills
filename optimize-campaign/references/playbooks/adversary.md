...
- **Measured Avoidable Share:** Open the bound `redundancy_evidence` packet and check that `estimated_avoidable_fraction` does not exceed its `applicable_fraction`/`repeat_fraction`, that the recomputed `estimated_local_story_impact_pct` clears the story's qualification floor (2 × calibrated MDE), and that the proposal is not shifting work to a downstream caller/microtask. A proposal whose fraction is typed rather than measured is REJECT.
- **Portability:** If the target entry carries a `platform_sensitivity` flag (rendering backend, font shaping, process plumbing), require a Pinpoint Mac M1 confirmation plan before any host time; otherwise CHALLENGE.
...
`benchmark_overfit_checked`, `feature_flag_guarded`, `runtime_binary_changed`,
and `portability_confirmed` (the mechanism's target entry is not flagged
platform-sensitive, or a Mac M1 Pinpoint result confirms it).
...
placeholder and the notes placeholder with a sentence naming the artifact you
opened (existing path or bound sha256 prefix) and the number read from it;
the importer rejects generic or repeated sentences. Put actionable issues in `findings`. PASS requires every check
