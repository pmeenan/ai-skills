    ...
    for role in ('skeptic', 'adversary'):
        ...
        if role == 'skeptic':
            ...
            rep['check_evidence'] = {
                ...
                'existing_reuse_examined': ('; '.join(f"row {i}: {str(paths[i-1].get('existing_mechanism') or paths[i-1].get('evidence') or '')[:220]}" for i in cands) + ' | symbols in tree: ' + '; '.join(f"{s} -> {hits[s]}" for s in sorted(syms))) if cands else f"no novel row at/above the floor; all {len(above)} rows close by count (0 novel)",
            }
        else:
            rep['checks'] = {k: True for k in rep['checks']}
            rep['check_evidence'] = {
