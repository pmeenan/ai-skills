#!/usr/bin/env python3
"""Tests for shared mechanism evidence."""
...
class MechanismEvidenceTest(unittest.TestCase):
    def test_scaffold_binds_default_avoidable_floor(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(0, evidence.main([
                "scaffold", "--benchmark", "speedometer3", "--opp", "7",
    ...
    def test_campaign_recomputes_artifact_instead_of_trusting_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            loaded, _ = campaign.load_gate_evidence(
                out, opp=opp, phase="sizing", benchmark="speedometer3",
                metric_model="speedometer-story-v1",
            )
            with self.assertRaisesRegex(campaign.CampaignError, "benchmark"):
                campaign.load_gate_evidence(
                    out, opp=opp, phase="sizing", benchmark="jetstream3",
                    metric_model="jetstream-workload-score-v1",
                )
            with self.assertRaisesRegex(campaign.CampaignError, "metric_model"):
                campaign.load_gate_evidence(
                    out, opp=opp, phase="sizing", benchmark="speedometer3",
                    metric_model="wrong-metric-model",
                )
