#!/usr/bin/env python3
# Shared campaign mechanism-evidence validator.
...
def validate_capture_manifest(ref: dict, metadata: dict, index: int) -> dict:
    ...
    for field in (
        "benchmark", "metric_model", "variant", "opportunity_id",
        "mechanism_key", "profile_id",
    ):
...
def mechanism_adapter(benchmark) -> benchmark_adapters.BenchmarkAdapter:
    """Resolve the bound adapter and fail closed where sizing is unavailable."""
    try:
...
def validate_raw(data: dict, path: pathlib.Path) -> list[dict]:
    ...
    adapter = mechanism_adapter(data.get("benchmark"))
    if data.get("metric_model") != adapter.metric_model:
        raise EvidenceError(
            f"{path}: metric_model must be {adapter.metric_model}"
        )
    for field in (
    ...
    if scope.get("metric_model") != adapter.metric_model:
        raise EvidenceError(
            f"{path}: score_scope.metric_model must be {adapter.metric_model}"
        )
...
def common_identity(left: dict, right: dict) -> None:
    for field in (
        "benchmark", "metric_model", "opportunity_id", "mechanism_key",
        "profile_id", "target_story", "interval_kind",
    ):
...
def base_output(data: dict, source_paths: list[pathlib.Path]) -> dict:
    return {
        ...
        "metric_model": data["metric_model"],
...
def cmd_scaffold(args: argparse.Namespace) -> None:
    adapter = mechanism_adapter(args.benchmark)
    out = {
        "schema_version": SCHEMA_VERSION,
...
def cmd_capture(args: argparse.Namespace) -> None:
    ...
    adapter = mechanism_adapter(args.benchmark)
    if metadata.get("benchmark") != adapter.benchmark_id:
        raise EvidenceError("capture benchmark does not match metadata")
    if metadata.get("metric_model") != adapter.metric_model:
        raise EvidenceError("capture metric model does not match metadata")
    root = pathlib.Path(git_value(pathlib.Path.cwd(), "rev-parse", "--show-toplevel"))
    source_tree = ensure_authoritative_index(root)
...
def parser() -> argparse.ArgumentParser:
    ...
    scaffold.add_argument(
        ...
        help="Benchmark workload the mechanism targets; sizing and "
        "verification run only this workload",
    )
